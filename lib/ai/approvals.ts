...
import { persistApprovalRow, persistChangeSetAudit, persistRunIfExists } from "./audit-persist";
...
export function assertExecutableApproval(
  ...
  // Once verified/rejected/expired the lifecycle is over. Partial-failure
  // states (executed/failed) stay retryable under the same approval.
  if (set.status === "verified" || set.status === "rejected" || set.status === "expired") {
    throw new AiError("approval", "Change set cannot be executed.", 403);
  }
  if (decision === "rejected") {
    if (set.status !== "pending" && set.status !== "approved") {
      throw new AiError("approval", "Change set cannot be rejected now.", 403);
    }
    approval.status = "rejected";
    approval.reviewed_at = new Date().toISOString();
    set.status = "rejected";
    memoryStore.saveApproval(approval);
    memoryStore.saveChangeSet(set);
    void persistApprovalRow(approval);
    void persistChangeSetAudit(set);
    void persistRunIfExists(set.agent_run_id, userId);
    return set;
  }
  approval.status = "approved";
  approval.reviewed_at = new Date().toISOString();
  set.status = set.status === "executed" || set.status === "failed" ? set.status : "approved";
  memoryStore.saveApproval(approval);
  memoryStore.saveChangeSet(set);
  void persistApprovalRow(approval);
  void persistChangeSetAudit(set);
