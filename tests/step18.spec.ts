...
async function createWorkspace(page: Page) {
  ...
  // The planner tests need real academic data in the workspace; the demo
  // workspace is seeded through the same repository path the product uses.
...
test("offline: AI is honest; local task persists in demo workspace", async ({ page, context }) => {
  ...
  // The queue is honest: reloading a private page while offline is not
  // claimed to work — the app shell is all the PWA caches. Persistence is
  // asserted once connectivity returns.
  await context.setOffline(false);
  await page.reload({ waitUntil: "domcontentloaded" });
  await expect(page.getByRole("button", { name: "Offline queued task", exact: true })).toBeVisible();
});

test("connectivity indicator reflects offline and returns to synced", async ({ page, context }) => {
  test.setTimeout(120000);
  await createWorkspace(page);
  await page.goto("/dashboard");
  const bar = page.locator("[data-connectivity]");
  await expect(bar).toBeVisible();
  await context.setOffline(true);
  // The browser 'offline' event drives the indicator to the offline state.
  await expect(bar).toHaveAttribute("data-connectivity", "offline", { timeout: 10000 });
  await expect(page.getByText(/Offline — changes saved locally/)).toBeVisible();
  await context.setOffline(false);
  await expect(bar).not.toHaveAttribute("data-connectivity", "offline", { timeout: 10000 });
});
...
test("demo accounts are isolated: user B never sees user A's data", async ({ page }) => {
  test.setTimeout(120000);
  await createWorkspace(page);
  await page.goto("/academics");
  await page.getByRole("button", { name: "Add task", exact: true }).first().click();
  const dialog = page.getByRole("dialog");
  await dialog.getByLabel(/^Title\s*\*?$/).fill("Only Alice's exam prep");
  await dialog.getByRole("button", { name: "Add task", exact: true }).click();
  await expect(
    page.getByRole("button", { name: "Only Alice's exam prep", exact: true }),
  ).toBeVisible();

  await page.getByRole("button", { name: "Sign out", exact: true }).click();
  await expect(page).toHaveURL(/login/);

  // User B: brand-new account.
  const email = `s18b-${Date.now()}-${Math.random().toString(36).slice(2, 6)}@example.test`;
  await page.goto("/register");
  await page.getByLabel(/^Email\s*\*?$/).fill(email);
  await page.getByLabel(/^Password\s*\*?$/).fill(PASSWORD);
  await page.getByLabel(/^Confirm\ password\s*\*?$/).fill(PASSWORD);
  await page.getByRole("button", { name: "Create account", exact: true }).click();
  await expect(page).toHaveURL(/onboarding/);
  await page.getByLabel(/^Full\ name\s*\*?$/).fill("Bobby Rao");
  await page.getByLabel(/^Course\s*\*?$/).fill("B.Tech");
  await page.getByLabel(/^Branch\ \/\ field\s*\*?$/).fill("Electronics");
  await page.getByRole("button", { name: "A little about your goals" }).click();
  await page.getByRole("button", { name: "I’ll think about my goals later" }).click();
  await expect(page).toHaveURL(/dashboard/);
