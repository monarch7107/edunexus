...
describe("step 20 hardening: durable ids, integrity, idempotency ledger", () => {
  const UUID_RE =
    /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;

  function planWithCreate(title: string): AgentPlan {
    return {
      summary: `add ${title}`,
      evidence: [],
      conflicts: [],
  ...
  it("generates uuid ids for every record stored in the Supabase agent tables", async () => {
    const orch = await runOrchestrator(
      { userId: USER_A },
      { message: "Fix my study schedule", snapshot: sampleContext() },
      { planFn: async () => planWithCreate("UUID check") },
    );
    expect(orch.runId).toMatch(UUID_RE);
    expect(orch.changeSetId).toMatch(UUID_RE);
    const set = memoryStore.getChangeSet(orch.changeSetId!, USER_A);
    expect(set?.id).toMatch(UUID_RE);
    expect(set?.items[0].id).toMatch(UUID_RE);
    const approval = memoryStore.getApprovalForSet(orch.changeSetId!, USER_A);
    expect(approval?.id).toMatch(UUID_RE);
  });

  it("proposal hash is a stable sha256 digest, key-order independent", () => {
    const a = hashChangeSet([
      { operation: "create", entity_id: null, payload: { title: "x", planned_date: "2099-01-01" } },
    ]);
    const b = hashChangeSet([
      { operation: "create", entity_id: null, payload: { planned_date: "2099-01-01", title: "x" } },
    ]);
    const c = hashChangeSet([
      { operation: "create", entity_id: null, payload: { title: "x", planned_date: "2099-01-02" } },
    ]);
  ...
  it("rejects the change set after execution without corrupting lifecycle state", async () => {
    const orch = await runOrchestrator(
      { userId: USER_A },
      { message: "Fix my study schedule", snapshot: sampleContext() },
      { planFn: async () => planWithCreate("Final status") },
    );
    const approved = await handleApproval({
      userId: USER_A,
      rawBody: { changeSetId: orch.changeSetId, decision: "approved" },
    ...
    expect(memoryStore.getChangeSet(orch.changeSetId!, USER_A)?.status).toBe("verified");
  });

  it("mutation ledger prevents duplication even if the status gate is reset", async () => {
    const orch = await runOrchestrator(
      { userId: USER_A },
      { message: "Fix my study schedule", snapshot: sampleContext() },
      { planFn: async () => planWithCreate("Ledger session") },
    );
    ...
    const before = memoryStore
      .getAcademic(USER_A)
      .sessions.filter((s) => s.title === "Ledger session").length;
    expect(before).toBe(1);

    // Simulate a stale state (crash recovery without the verified status).
    const set = memoryStore.getChangeSet(orch.changeSetId!, USER_A)!;
    set.status = "approved";
    set.items.forEach((i) => (i.status = "pending"));
    memoryStore.saveChangeSet(set);

    const again = await executeApprovedChangeSet(USER_A, orch.changeSetId!);
